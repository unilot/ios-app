...
//      button.addTarget(self, action: #selector(HistoryTable.onContractId(sender:)), for: .touchUpInside)
    ...
    func fillSegmentNames(){
        ...
        for i in 1..<6 {
            ...
            if currentTable != (i-1) {
                imageView.tintColor = kColorHistoryUnselectDarkGray
                label.textColor = kColorHistoryUnselectDarkGray

            } else {
                imageView.tintColor = kColorHistoryDarkGray
                label.textColor = kColorHistoryDarkGray
